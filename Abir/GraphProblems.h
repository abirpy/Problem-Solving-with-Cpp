...
public:
    ...
    cur){
        ...
        // after the recursive during backtracking stack e push korbo
        s.push(cur);
    }
    ...
    GraphN cur){
     ...
     // djikra is like floyd warshall but with a single src
     // for every j we are trying to minimise adj[src][j]
     // O(V^2)
     void djikstraWithAdjMatrix(vector<vector<int>>& adj, int src){
         for(int k = 0; k < adj.size(); ++k){
             for(int j = 0; j < adj.size(); ++j) {
                 if (adj[src][k] == INT32_MAX || adj[k][j] == INT32_MAX) {
                     continue;
                 } else if (adj[src][k] + adj[k][j] < adj[src][j]) {
                     adj[src][j] = adj[src][k] + adj[k][j];
                 }
             }
         }
    }
    ...
    // a modified BFS where you continuously remove nodes of degree 0 from the graph until the
    // queue size is <= 2
